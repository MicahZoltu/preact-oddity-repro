...
export function UniswapAndSend(model: UniswapAndSendModel) {
	...
	const SourceToken = (sourceAmountResult.state === 'pending') ? <Spinner/> : <TokenAndAmount token={sourceToken} amount={sourceAmount} onChange={sourceChanged}/>
	const TargetToken = (targetAmountResult.state === 'pending') ? <Spinner/> : <TokenAndAmount token={targetToken} amount={targetAmount} onChange={targetChanged}/>
